...
visits <- main_record %>% 
  dplyr::select(`ROMS ID`,Visits) %>% 
  dplyr::distinct()

visits %<>% 
  dplyr::filter(!is.na(Visits) & !is.na(`ROMS ID`)) %>%
  dplyr::group_by(`ROMS ID`) %>%
  dplyr::summarise(Visits = max(Visits)) %>%
  dplyr::ungroup()
...
md_data_new_1 <- dplyr::left_join(md_data_new_1,visits,by = "ROMS ID") 
